package com.mobile.newFramework.pojo;

/**
 * Class used to identify the Error keys.
 */
public class ErrorConstants {

    public final static String CUSTOMER_NOT_LOGGED_IN = "CUSTOMER_NOT_LOGGED_IN";
    public final static String CUSTOMER_CREATE_FAILED_EXISTS = "CUSTOMER_CREATE_FAILED_EXISTS";
    public final static String ORDER_PRODUCT_SOLD_OUT = "ORDER_PRODUCT_SOLD_OUT";
    public final static String PRODUCT_ADD_OVER_QUANTITY = "SR_ORDER_PRODUCT_ERROR_ADDING_STOCK_ABOVE_ALLOWED_QUANTITY";
    public final static String ORDER_PRODUCT_ERROR_ADDING = "ORDER_PRODUCT_ERROR_ADDING";
    public final static String ERROR_ADDING_ITEM = "ERROR_ADDING_ITEM";

}
