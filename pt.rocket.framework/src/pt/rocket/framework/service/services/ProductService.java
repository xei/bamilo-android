...
public class ProductService extends DarwinService {
	...
	public ProductService() {
		super(EnumSet.noneOf(EventType.class), EnumSet.of(EventType.INIT_SHOP,
				EventType.GET_PRODUCTS_EVENT, EventType.GET_PRODUCT_EVENT,
				EventType.GET_SEARCH_SUGGESTIONS_EVENT,
				EventType.GET_PRODUCT_REVIEWS_EVENT,
				EventType.REVIEW_PRODUCT_EVENT,
	...
	public void getProduct(final GetProductEvent event) {
		...
		RestServiceHelper.requestGet(event.value,
				new ResponseReceiver<CompleteProduct>(event) {

					@Override
					public CompleteProduct parseResponse(JSONObject metadataObject)
	...
	/**
	 * Function that fetches the all the fields suggestions for a certain query. Warning. the query
	 * must have at least 1 character.
	 * 
	 * @param event
	 */
	public static void getSearchAutoComplete(
			final GetSearchSuggestionsEvent event) {

		Uri uri = Uri.parse(event.eventType.action)
				.buildUpon().appendQueryParameter("q", event.value).build();

		RestServiceHelper.requestGet(uri, new ResponseReceiver<List<SearchSuggestion>>(
				event) {

			public List<SearchSuggestion> parseResponse(JSONObject metadataObject)
					throws JSONException {
	...
	public static void getProducts(final GetProductsEvent event) {
		...
		RestServiceHelper.requestGet(uriBuilder.build(),
				new ResponseReceiver<ProductsPage>(event) {

					@Override
					public ProductsPage parseResponse(JSONObject metadataObject)
							throws JSONException {
		...
		if (null == ratingOptions) {
			...
			RestServiceHelper
					.requestGet(
							...
							new ResponseReceiver<HashMap<String, HashMap<String, String>>>(
									event) {

								@Override
								public HashMap<String, HashMap<String, String>> parseResponse(
									...
									for (int i = 0; i < size; i++) {
										ratingOption = dataArray
												.getJSONObject(i);
										JSONArray optionsArray = ratingOption
												.getJSONArray(JSON_OPTIONS_TAG);
										option = new HashMap<String, String>();
										optionsSize = optionsArray.length();
										for (int k = 0; k < optionsSize; k++) {
											optionObject = optionsArray
													.getJSONObject(k);
											option.put(
													optionObject
															.getString(JSON_VALUE_TAG),
													optionObject
															.getString(JSON_ID_RATING_OPTION_TAG));
										}
										ratingOptions.put(ratingOption
												.getString(JSON_CODE_TAG),
												option);
									}
	...
	private void sendRatingOptionsResult(RequestEvent event) {
		EventManager.getSingleton().triggerResponseEvent(
				new ResponseResultEvent<Map<String, ? extends Map<String, String>>>(event,
						ratingOptions, null, new Bundle()));
	}
	 ...
	 * @param pageNumber
	 */
	public static void getProductReviews(
			final GetProductReviewsEvent event) {

		Uri uri = Uri
				.parse(event.productUrl)
				.buildUpon()
				.appendQueryParameter(RestContract.REST_PARAM_RATING, "1")
				.appendQueryParameter(RestContract.REST_PARAM_PAGE,
						String.valueOf(event.pageNumber))
				.build();

		RestServiceHelper.requestGet(uri,
				new ResponseReceiver<ProductRatingPage>(event) {

					@Override
					public ProductRatingPage parseResponse(JSONObject metadataObject)
							throws JSONException {
	...
	public void reviewProduct(final ReviewProductEvent event) {
		...
		for (Entry<String, HashMap<String, String>> option : ratingOptions.entrySet()) {
			values.put(
					REVIEW_OPTION_FIELD + option.getKey(),
					option.getValue().get(
							String.valueOf((int) event.productReviewCreated.getRating())));
		}
