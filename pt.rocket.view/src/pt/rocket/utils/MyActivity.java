 ...
 * 
 */
public abstract class MyActivity extends RobotoActivity implements OnOpenedListener, OnClosedListener, ResponseListener {
...
//	private int navigationComponentsHashCode;
	...
	private void setupNavigationMenu() {
        ...
        if (navigationGenerator == null) {
            navigationGenerator = new RocketNavigationListGenerator(getApplicationContext());
        }
	}
...
//        getSlidingMenu().setMenu(navigationContainer);
     ...
     */
    private void updateCartInfo(ShoppingCart cart) {
        if (cart != null) {
            Log.d(getTag(),
	...
	private void updateCartInfoInNavigation(final ShoppingCart cart) {
        ...
        View container = navigationContainer.findViewById(R.id.nav_basket);
        if (container == null) {
        ...
        final String value = cart != null ? cart.getCartValue() : "";
        final String quantity = cart == null ? "?" : cart.getCartCount() == 0 ? "" : String
                .valueOf(cart.getCartCount());
    ...
    private void showError(final RequestEvent event) {
    ...
    private void handleErrorEvent(final ResponseEvent event) {
        if (event.errorCode.isNetworkError()) {
            ...
            if (contentEvents.contains(event.type)) {
                ...
                showContentContainer();
                dialog = DialogGeneric.createNoNetworkDialog(MyActivity.this,
                        new OnClickListener() {
            ...
            showContentContainer();
            dialog = new DialogGeneric(MyActivity.this,
